...
it('로딩이 완료된 경우 상품 리스트가 제대로 모두 노출된다', async () => {
  await render(<ProductList limit={PRODUCT_PAGE_LIMIT}></ProductList>);
...
it('보여줄 상품 리스트가 더 있는 경우 show more 버튼이 노출되며, 버튼을 누르면 상품 리스트를 더 가져온다.', async () => {
  const { user } = await render(
    <ProductList limit={PRODUCT_PAGE_LIMIT}></ProductList>,
  );
  // show more 버튼의 노출 여부를 정확하게 판단하기 위해
  // findBy 쿼리를 사용하여 먼저 첫 페이지에 해당하는 상품 목록이 렌더링되는 것을 기다려야함
  ...
  const moreBtn = screen
    .getAllByRole('button', { name: 'Show more' })
    .toBeInTheDocument();
...
it('보여줄 상품 리스트가 없는 경우 show more 버튼이 노출되지 않는다.', async () => {
  // 모킹 데이터 20개보다 많은수 50으로 limit을 설정
  await render(<ProductList limit={50}></ProductList>);

  //findBy 쿼리를 사용하여 먼저 첫 페이지에 해당하는 상품 목록이 렌더링 되는것을 기다려야함
  await screen.findAllByTestId('product-card');
