...
it('카테고리 목록을 가져온 후 카테고리 필드의 정보들이 올바르게 렌더링된다.', async () => {
  await render(<ProductFilter></ProductFilter>);
...
it('상품명을 수정하는 경우 setTitle 액션이 호출된다.', async () => {
  const { user } = await render(<ProductFilter></ProductFilter>);
...
it('카테고리를 클릭 할 경우의 클릭한 카테고리가 체크된다.', async () => {
  // 라디오클릭 => setCategoryId => categoryId state변경 => 선택된 라디오 값 변경
  const { user } = await render(<ProductFilter></ProductFilter>);

  const category3 = await screen.findByLabelText('caategory3');
